/**
 * Envoie le devis exposant à Farida Kibout (Nyukki Miels) par email avec PDF en pièce jointe.
 * Usage : npx tsx scripts/send-devis-nyukki.ts
 */
...
async function main() {
  ...
  const { data, error } = await resend.emails.send({
    from: FROM,
    to: "farida.kibout@hotmail.fr",
    subject: "Devis Exposant — Foire d'Afrique Paris 2026 — Nyukki Miels",
    html: `<!DOCTYPE html>
<html lang="fr">
...
<body style="font-family: Georgia, serif; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>Bonjour <strong>Farida</strong>,</p>

  <p>Merci pour votre candidature et ravi de vous revoir suite à votre rencontre avec Mme Koffi ! <strong>Nyukki Miels</strong>, c'est une belle marque que nos visiteurs vont adorer découvrir.</p>
  ...
  <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #e5e5e5; font-size: 12px; color: #999;">
    ...
    attachments: [
      {
        filename: "Devis-Nyukki-Miels-Foire-Afrique-2026.pdf",
        content: buffer,
      },
    ],
  });
  ...
  console.log(`✓ Email envoyé à farida.kibout@hotmail.fr — ID: ${data?.id}`);
}
